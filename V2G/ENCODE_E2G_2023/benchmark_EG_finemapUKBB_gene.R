...
benchmark_EG_finemapUKBB_gene = function(EGpred_file,
  ...
  bimlist = list()
  for(numchr in 1:22){
    xx = data.frame(fread(paste0("/n/groups/price/kushal/extras/BIMS_hg38/", "1000G.EUR.QC.", numchr, ".bim")))
  ...
  pops_silver = data.frame(fread("/n/groups/price/kushal/ENCODE/data/PoPS/UKBiobank.ABCGene.anyabc.tsv"))
  if(tissue == "ALL"){
    pops_silver = pops_silver
  }else if (tissue == "BLD"){
    traits = c("Eosino", "Lym", "Mono", "Neutro", "Plt", "RBC", "MCV", "Hb", "HbA1c", "MCH", "MCHC")
  ...
  for(numu in 1:length(ucreds)){
    ...
    if(length(cc) > 0){
      ...
      if(max(gene_scores2) == 0){
        ...
        genes_selected = names(gene_scores2)[which(gene_scores2  == max(gene_scores2))]
        if(use_pops){
